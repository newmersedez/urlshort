...
// makeStatsRouter создаёт полный роутер с заданной подсетью —
func makeStatsRouter(t *testing.T, cidr string, store *mocks.MockRepository) http.Handler {
	t.Helper()
	tokenService := middlewareMocks.NewMockTokenService(t)
	tokenService.On("IsValid", mock.Anything).Return(false).Maybe()
	tokenService.On("GetToken", mock.Anything).Return("test-token", nil).Maybe()
	cleanupService := mocks.NewMockCleanupService(t)
	auditService := mocks.NewMockAuditService(t)
	h, err := newHandlers("http://localhost:8080", parseCIDROrNil(cidr), store, slog.Default(),
...
func TestGetStatsHandle(t *testing.T) {
	...
	t.Run("returns stats for trusted IP", func(t *testing.T) {
		...
		router := makeStatsRouter(t, trustedCIDR, mockRepo)
	...
	t.Run("returns 403 when trusted_subnet is not set", func(t *testing.T) {
		mockRepo := mocks.NewMockRepository(t)
		router := makeStatsRouter(t, "", mockRepo)
